...
@testset "Callback function             " begin
    ...
    @test_nowarn Callback(rand(5), rand(5), rand(Int, 5), rand(5))
    @test_throws ArgumentError Callback(rand(5), rand(3), rand(Int, 5), rand(5))

    # test trace assignment
    trace = Callback()
    value = rand(); g_norm = rand(); iter = rand(1:10); metadata = Dict("time"=>rand())
