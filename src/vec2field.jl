...
function field2vec!(vec::Vector{T}, u::SpectralField{Ny, Nz, Nt, <:Any, T}) where {T, Ny, Nz, Nt}
...
function field2vec!(vec::Vector{T}, u::VectorField{N, <:SpectralField{Ny, Nz, Nt, <:Any, T}}) where {T, N, Ny, Nz, Nt}
    ...
    for i in 1:N
        field2vec!(@view(vec[((i - 1)*Ny*Nz_spec*Nt + 1):i*Ny*Nz_spec*Nt]), u[i])
    end

    return vec
end

function vec2field!(u::SpectralField{Ny, Nz, Nt, <:Any, T}, vec::Vector{T}) where {Ny, Nz, Nt, T}
...
function vec2field!(u::VectorField{N, <:SpectralField{Ny, Nz, Nt, <:Any, T}}, vec::Vector{T}) where {N, Ny, Nz, Nt, T}
    ...
    # loop over the elements of the state vector and assign to the field
    for i in 1:N
        vec2field!(u[i], @view(vec[((i - 1)*Ny*Nz_spec*Nt + 1):i*Ny*Nz_spec*Nt]))
    end
