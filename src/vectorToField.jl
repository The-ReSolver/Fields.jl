...
function _velocityCoefficientsToVector!(vector, velocityCoefficients::SpectralField{<:Grid{Ny, Nz, Nt}, true}) where {Ny, Nz, Nt}
    NzSpectral = size(velocityCoefficients, 2)
    for nt in 1:Nt, nz in 1:NzSpectral, ny in axes(velocityCoefficients, 1)
