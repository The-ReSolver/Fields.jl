...
function fieldToVector!(vector, velocityCoefficients::SpectralField{<:Grid{Ny, Nz, Nt}, true}, ω) where {Ny, Nz, Nt}
...
function vectorToField!(velocityCoefficients::SpectralField{<:Grid{Ny, Nz, Nt}, true}, vector) where {Ny, Nz, Nt}
    M = size(velocityCoefficients, 1)
    for nt in 1:Nt, nz in 1:((Nz >> 1) + 1), m in 1:M
